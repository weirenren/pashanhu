...
let capture_type = FULL_DAY_CAPTURE;
...
function getBaseUrl(page) {
    return 'http://muchong.com/f-127-'+ page +'-typeid-1074'
}
...
function doLoopWorkForTotal(next) {
    ...
    let last_page_start = 2
    cratchMainPage(getBaseUrl(last_page_start), (errno, err_str) => {
        if (errno == -1) {
            ongoingPostUrlList.push(getBaseUrl(last_page_start))
        }
...
function doWork(next) {
    var xiaomuchong_diaoji_url = 'http://muchong.com/f-430-1'

    cratchMainPage(xiaomuchong_diaoji_url, next)
}
...
function cratchMainPage(url, next) {
    ...
    superagent.get(url)
        ...
        .end(function (error, data) {
            ...
            $('.bg .wrapper ').each(function (idx, element) {
                // var tb = $(element).html();
                ...
                var href = $(element).children('tr').first().children('th').eq(1).children('a').first().attr('href')
                console.log("user href:" + href);
                if (href) {
                    ongoingPostUrlList.push(href)
                }
...
Promise.resolve()
    .then(cratchMainPage('http://muchong.com/f-127-2-typeid-1074', () => {}));
    // .then(catch_list);
    //  .then(dropIndex)
    // .then(reset)
